...
function render() {
  ...
  initDrag(); updateChart(); document.body.classList.remove('show-modal');
}
...
function initDrag() {
  let dragged;
  qa('.card').forEach(card => { card.ondragstart = () => { dragged = card; }; card.ondragend = () => {}; });
  qa('.cards').forEach(zone => { zone.ondragover = e => e.preventDefault(); zone.ondrop = () => {
      const id = +dragged.dataset.id; const client = crm.clients.find(c => c.id === id);
      client.stage = zone.dataset.stage; crm.update(client); render();
  }; });
}

let editId = null;
qs('#btn-add').onclick = () => { editId = null; qs('#modal-title').textContent = 'New Client'; qs('#client-form').reset(); qs('#modal').classList.remove('hidden'); };
qs('#btn-cancel').onclick = () => qs('#modal').classList.add('hidden');

qs('#client-form').onsubmit = e => {
  e.preventDefault(); const c = {
    id: editId, name: qs('#name').value.trim(), email: qs('#email').value.trim(),
    phone: qs('#phone').value.trim(), tags: qs('#tags').value.split(',').map(s=>s.trim()).filter(Boolean),
    stage: qs('#stage').value, notes: qs('#notes').value.trim()
  };
  editId ? crm.update(c) : crm.add(c); qs('#modal').classList.add('hidden'); render();
};
...
qs('#btn-export').onclick = () => { const blob = new Blob([toCSV(crm.clients)], {type:'text/csv'}); const url = URL.createObjectURL(blob); const a = document.createElement('a'); a.href=url; a.download='clients.csv'; a.click(); };
qs('#btn-import').onclick = () => qs('#importFile').click();
qs('#importFile').onchange = e => { const file = e.target.files[0]; if (!file) return; const reader = new FileReader(); reader.onload = () => {
    const [hdr, ...rows] = reader.result.trim().split('\n'); const keys = hdr.split(',');
    rows.forEach(r => { const vals = r.match(/(".*?"|[^,]+)/g).map(s=>s.replace(/^"|"$/g,'').replace(/""/g,'"')); const obj = {}; keys.forEach((k,i)=>obj[k]=vals[i]); obj.tags = obj.tags.split(';').map(s=>s.trim()).filter(Boolean); crm.add(obj); }); render();
...
function updateChart() {
  ...
  else { chart = new Chart(ctx, { type:'pie', data:{labels:crm.STAGES, datasets:[{data}]}, options:{responsive:true} }); }
}
