class CRM {
  constructor() {
    ...
    // Ensure fresh on very first load:
...
function initDrag() {
  let drag;
  qa('.card').forEach(cd=>cd.ondragstart=()=>drag=cd);
  qa('.cards').forEach(zone=>{
    zone.ondragover=e=>e.preventDefault();
    zone.ondrop=()=>{
      const id=+drag.dataset.id;
      const client=crm.clients.find(x=>x.id===id);
...
qs('#importFile').onchange=e=>{
  ...
  r.onload=()=>{
    ...
    rows.forEach(rw=>{
      const vals=rw.match(/(".*?"|[^,]+)/g).map(s=>s.replace(/^"|"$/g,'').replace(/""/g,'"'));
      const obj={};
